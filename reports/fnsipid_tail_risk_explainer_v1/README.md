...
| 2 | **Forecasting starts only at the mapped reaction-session close**<br>[`figures/e02_timing_rule.png`](figures/e02_timing_rule.png) | The date-only branch checked all 4,779 calendar dates with 0 non-forward mappings. Original event timestamps are absent from the checkpoint, so the branch used by each retained headline cannot be re-verified. |
...
| 9 | **The price archive has candidate adjustment discontinuities that may amplify tail losses**<br>[`figures/e09_price_adjustment_integrity.png`](figures/e09_price_adjustment_integrity.png) | 470 of 1,788,865 firm-days (0.026%) are flagged, and they breach at 66% against 3.0% on unflagged days. Common targets enter each model's nonlinear FZ0 loss differently, so they can move M2-vs-M1; the declared repair check moved the estimate but its interval still spanned zero. |
...
| 11 | **All four models are equally under-conservative — the ranking survives, absolute accuracy does not**<br>[`figures/e11_calibration.png`](figures/e11_calibration.png) | Breach rate is about 3.06% against a nominal 2.5%. The M2-vs-M1 comparison is paired on common rows, but nonlinear FZ0 losses need not cancel a shared scale bias; this is a relative ranking, not a deployable risk system. |
| 12 | **The volatility filter is stretched, but this diagnostic does not identify one cause**<br>[`figures/e12_volatility_filter.png`](figures/e12_volatility_filter.png) | Overall std(z) is 1.117, and 2020 reaches 1.40. This is consistent with under-forecast conditional scale, but does not prove that scale is the only source of over-breaching. |
