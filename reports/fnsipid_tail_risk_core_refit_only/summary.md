...
- Variant: **refit_only** — price repair `none`, volatility refit `annual_expanding`
  Relative to v1, this cell re-estimates the volatility filter before each evaluation year on an expanding window. Nothing else differs.
- Run mode: **full**
...
difference `FZ0(M2) - FZ0(M1)` is **-0.000351434** with a 95% date-block
bootstrap interval of **[-0.00228369, +0.00128429]** (61.6% of bootstrap means below zero).
In relative terms mean loss changes by -0.0293%; a negative paired
...
| Quantity | Value |
| --- | ---: |
| mean FZ0, M0, news-bearing origins | 1.210128 |
| mean FZ0, M1, news-bearing origins | 1.201192 |
| mean FZ0, M2, news-bearing origins | 1.200841 |
| mean FZ0, M2_intensity, news-bearing origins | 1.200534 |
| paired_fz0_M2_minus_M1 | -0.000351434 |
| paired_fz0_M2_minus_M2_intensity | +0.000307138 |
| paired_fz0_M1_minus_M0 | -0.00893571 |
| relative_fz0_improvement_pct_M2_vs_M1 | +0.0292571 |
| VaR hit rate, M0, full panel (nominal 0.025) | 0.03010 |
| VaR hit rate, M1, full panel (nominal 0.025) | 0.02979 |
| VaR hit rate, M2, full panel (nominal 0.025) | 0.02986 |
| VaR hit rate, M2_intensity, full panel (nominal 0.025) | 0.02985 |
...
panel M1 breaches on 0.02979 of days against a nominal 0.025, and the 95%
date-block interval on the excess, [+0.00114, +0.00978], excludes zero.
8 of 8 DQ conditional-coverage and ES identification tests reject at 5%.
Evaluation-period standardised returns have standard deviation 1.101, so the
...
paired difference moves from -0.000351434 to -0.000996472 (95% interval [-0.00500025, +0.00138491]).
...
```bash
REPRO_DIR="reports/reproductions/fnsipid_tail_risk_core_refit_only_$(date -u +%Y%m%dT%H%M%SZ)"
FNSPID_TAIL_RISK_RUN_MODE=full \
FNSPID_TAIL_RISK_VARIANT=refit_only \
