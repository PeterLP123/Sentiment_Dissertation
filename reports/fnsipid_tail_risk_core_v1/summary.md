...
- Variant: **v1** — price repair `none`, volatility refit `frozen_development`
- Run mode: **full**
...
difference `FZ0(M2) - FZ0(M1)` is **-0.000306678** with a 95% date-block
bootstrap interval of **[-0.00224877, +0.0013156]** (60.1% of bootstrap means below zero).
In relative terms mean loss changes by -0.0253%; a negative paired
...
| Quantity | Value |
| --- | ---: |
| mean FZ0, M0, news-bearing origins | 1.222823 |
| mean FZ0, M1, news-bearing origins | 1.213105 |
| mean FZ0, M2, news-bearing origins | 1.212798 |
| mean FZ0, M2_intensity, news-bearing origins | 1.212525 |
| paired_fz0_M2_minus_M1 | -0.000306678 |
| paired_fz0_M2_minus_M2_intensity | +0.000273778 |
| paired_fz0_M1_minus_M0 | -0.00971818 |
| relative_fz0_improvement_pct_M2_vs_M1 | +0.0252804 |
| VaR hit rate, M0, full panel (nominal 0.025) | 0.03090 |
| VaR hit rate, M1, full panel (nominal 0.025) | 0.03066 |
| VaR hit rate, M2, full panel (nominal 0.025) | 0.03065 |
| VaR hit rate, M2_intensity, full panel (nominal 0.025) | 0.03066 |
...
panel M1 breaches on 0.03066 of days against a nominal 0.025, and the 95%
date-block interval on the excess, [+0.00206, +0.01042], excludes zero.
8 of 8 DQ conditional-coverage and ES identification tests reject at 5%.
Evaluation-period standardised returns have standard deviation 1.117, so the
...
```bash
REPRO_DIR="reports/reproductions/fnsipid_tail_risk_core_v1_$(date -u +%Y%m%dT%H%M%SZ)"
FNSPID_TAIL_RISK_RUN_MODE=full \
FNSPID_TAIL_RISK_VARIANT=v1 \
