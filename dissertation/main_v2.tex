...
\draftnote{Three bounded contributions: (1) a like-for-like comparison of nine aggregation rules under one inference regime on 715{,}546 firm-days; (2) the separation of statistical detectability from economic usability, with a cost frontier; (3) evidence that the surviving negative-tail signal works as a downside-risk state rather than a return signal, with a placebo test that draws the boundary of the claim.}
...
\draftnote{Daily cross-sectional Spearman rank correlation against next-open abnormal returns, HAC-adjusted $t$-statistics on the daily series, Benjamini--Hochberg correction across the nine-rule family, and five-session block bootstraps for all strategy comparisons.}
...
\draftnote{One compact section on the non-pooled LSEG regimes: the five-arm sector-33 and midcap-22 nulls, and the temporal-fragility result. The hybrid selected on the opened 2025--2026 window earned net Sharpe 2.271 there and $-0.354$ on an earlier block of the same 33 names; the portable continuous rule was net-negative in both blocks while headline-level scores stayed highly rank-repeatable (Spearman 0.989). Stable measurement does not by itself produce a stable, cost-covering signal. Sources: Notebooks 11, 13, 36, 67, 70.}
...
\draftnote{Answer the primary and secondary questions directly with the main estimates and their uncertainty, then state the composite conclusion: the distribution beyond the mean carries detectable negative-tail information; it is not economically usable as a directional signal; it has bounded value as a downside-risk state.}
