...
The distinct novelty studies numbered 85 and 86 were added to Git on 8 September
2026 from existing executed notebooks, paired sources and aggregate outputs.
No study was rerun for publication. Notebook 86's evaluation is retrospective,
and historical references to a "mechanism" describe exploratory diagnostics,
not an established causal explanation. These studies remain outside the submitted
package. See the [archive guide](../docs/research_archive.md) for their status.
...
| Final RQ | Selected at Gate F1 on 2026-08-14: does negative-news share add predictive information for next-session abnormal returns beyond mean continuous sentiment and news volume, and is that increment stable across time? No separate secondary RQ. |
| Beyond-mean result | Notebook 71's FNSPID development coefficient is −0.00831 with 95% HAC CI [−0.01410, −0.00252] (p=0.0049), and Notebook 79's frozen price-path control strengthens it to −0.00914 [−0.01466, −0.00362] (p=0.00118). Notebook 75 then opens the 2020–2023 block once: the all-day coefficient is +0.00583 [−0.00426,+0.01593] (BH q=0.515), the n≥2 coefficient is +0.00295 [−0.00990,+0.01581] (q=0.653), and no nine-rule IC survives BH. The predeclared evaluation-minus-development contrast is +0.01414 [+0.00250,+0.02579] (p=0.0173). The result is temporal non-replication/sign instability, not a stable beyond-the-mean effect. Both primary LSEG intervals also cross zero. |
| New versus already-known news | Notebook 85 asks, before any return is loaded, whether the selected RQ's information sits in genuinely new stories or in restatements. At the return-blind Jaccard-0.50 screen, 9.46% of development stories repeat a strictly earlier same-firm headline. Of three declared tests, only the already-known **share** survives BH: −0.00882 [−0.01387,−0.00376], q=0.0019, added to mean sentiment, negative share and log news count. The mean-sentiment contrast is null and wrong-signed (−0.0152, p=0.226); the negative-share contrast has the declared sign at −0.0422 [−0.0821,−0.0024] but q=0.0565. Its marginal decomposition is the sharper statement — the negative-share relationship is −0.0234 (p=0.0090) on the new leg and +0.0029 (p=0.795) on the already-known leg — while the paired difference itself is imprecise. Development-only, uncalibrated screen, and the rolling contrasts drift toward zero after 2016 in the same direction as Notebook 75's non-replication. Treat as input-side conditioning evidence, not a validated effect. |
| Is the already-known share alpha? | No. Notebook 86 prices the T3 coefficient and 1 of 5 declared gate conditions passes. The quintile sort is not monotone — only the most-repeated quintile moves, at −1.91 bps, and Q5−Q1 is −1.93 bps/session [−3.83,−0.10] against a 20-bps round trip. Development gross Sharpe is 1.040 raw / 0.707 conditional-residual, but break-even is 0.96 / 0.49 bps/side versus the 10 charged, so net Sharpe is −9.75 / −13.65 and both books cross zero below 1 bp/side. The statistical mechanism is nonetheless real and specific: it survives Notebook 79's price-path controls (−0.00774, p=0.0024) so it is not reversal, and M1 places it in repeated **non-negative** stories (−0.00962, p=0.00049) rather than repeated negative ones (p=0.810), which points at stale low-information days rather than restated bad news. The unchanged retrospective replay on the already-opened 2020–2023 block flips the coefficient to +0.00337 (p=0.408) and loses gross. Record as a cost-infeasible, non-persistent development regularity. |
| Exact risk-state transfer | Notebook 72 is a zero-activation recent-window result. Notebook 73 freezes the same rule before acquiring older LSEG index returns and observes 53/456 risk-off sessions. The backward overlay raises net Sharpe from 1.291 to 1.520 and passes the downside and circular-timing families, but its return interval crosses zero, its second-half return effect is negative, drawdown improves only 3.63%, and the full useful-risk/cross-source gates fail. Treat this as partial downside-timing portability, not strategy validation. |
...
| LSEG prompt-engineering search | Notebook 83 scores five fixed target-company Reuters prompts on 9,317 company-session events for $2.97091685. Every prompt retains at least 99% strict output; the five-way strict intersection is 9,195 events (98.69%). The original 99% common-sample gate is preserved as a pre-return NO-GO; a disclosed post-score/pre-return v2 uses the unchanged strict intersection. All five 2024 prompt portfolios lose before costs. The least bad, delayed-reaction h5, has gross/net means of −0.99/−4.01 bps per session, net Sharpe −1.10 and −9.96% ending return. No prompt is eligible and no 2025 prompt portfolio is computed. A first-execution boundary incident materialised event-level 2025 returns without computing a portfolio; the corrected final run materialises none. This is an economically clear prompt-engineering null, not evidence that the generic scorer is optimal. |
| Structured anticipated-reaction score | Notebook 84 tests the researcher's requested structure without a novelty judgement or earlier headlines: target specificity, better/worse/unclear versus expectations, temporary/persistent/unclear, down/flat/up first reaction, then a −1 to +1 directional sentiment score. Chronological scoring stops after 3,241/3,242 strict 2024 events for $0.18951202. The h1 book is highly active (234 formation sessions; 215.16 annualised turnover) and loses before costs: −2.59 gross and −19.67 net bps/session, net Sharpe −2.18, and −40.53% net ending return. It is positive and beats each same-date scorer only in Q4. The gate fails, so 2025 and 2026 are not scored and no 2025 return is materialised. |
...
3. **Exploratory secondary arms**: story type, earnings, pooled learned thresholds, and all three non-pooled LSEG regimes have now been run. See [`EXPLORATORY_EXPERIMENT_LEDGER.md`](EXPLORATORY_EXPERIMENT_LEDGER.md).
   ...
   through Notebook 73. The sentiment-free HAR target remains the research
   ...
   deployment-qualified strategy. Across twenty paired sentiment/construction comparisons,
   one favourable return interval excludes zero and seven report lower downside
   ...
   bps/side in all seven regime/split rows. This freezes the useful result as
   ...
   metadata-filter variants are permitted. Notebook 67 now shows the equal-weight
   hybrid is negative in the earlier same-33 block, so retain its unchanged
   prospective contract only as a decisive future falsification opportunity;
   do not retune it historically or describe it as the leading validated base.
   ...
   Notebook 67 then joins the fully scored backward same-33 block to the
   original-33 portion of the recent 44-company block without retuning the
   Notebook 36 hybrid. The return-blind join contains 1,967,554 unique
   normalized headlines; earliest occurrences and scores are retained across
   16,444 cross-block overlaps. Eleven incompletely collected backward
   firm-open sessions are forced to cash under an amendment frozen before any
   strategy result was observed. On 445 complete backward sessions (456 total,
   94 active), gross/net Sharpe is 0.691/−0.354, net return is −3.43%, and
   break-even is 6.60 bps/side at the frozen 10-bps cost. The cash interval is
   [−3.47,+2.03] bps/session (p=0.6122), the sector-spanning alpha interval is
   [−3.55,+1.43] bps/session (p=0.4034), neither mechanism test passes BH, and
   only 1/33 company and 3/11 sector exclusions remain profitable. The joined
   descriptive path has gross/net Sharpe 1.639/0.467 and +6.51% net return over
   623 sessions, but this masks the negative earlier block and the positive
   already-opened recent block. The original 0.7881-versus-0.80 Gemma drift
   failure is not rescinded, so this is a post-stop earlier-time sensitivity,
   not the preregistered replication, independent confirmation, or a licence
   to retune. It materially weakens the historical strategy case.
   Notebook 70 then replays Notebook 39's unchanged mean-continuous rule over
   both same-source time blocks. The backward block has gross/net Sharpe
   0.308/−1.159, −15.20% net return and 2.09-bps/side break-even; its cash
   interval [−7.71,+0.90] bps/session crosses zero. The opened block is worse:
   gross/net Sharpe −1.409/−2.391, −17.45% net return and −14.02-bps/side
   break-even. Its cash interval [−20.79,−2.48] is negative after BH correction
   (q=0.033), its sector-factor interval is wholly negative, and all four
   block-half net Sharpes are negative. The joined 623-session path loses
   ...
   Notebook 79 then addresses the most direct omitted-variable objection on
   development only. On the identical 511,941-row price-complete sample, the
   baseline coefficient is −0.00830. Adding within-day ranks of the firm's
   open-to-open return ending at formation, its cumulative five-session return,
   and 20-session realised volatility strengthens the coefficient to −0.00914
   [−0.01466,−0.00362] (p=0.00118). Every control ends at the open where the
   outcome begins, so there is no return overlap. This rejects the simple
   reversal/lagged-momentum account within development; it is retrospective
   robustness, not causality or independent confirmation.
   Gate F1 was then taken on 2026-08-14 before Notebook 75 opened any declared
   evaluation statistic. Notebook 75 ran once under its parent freeze and
   outcome-blind amendment. Across 998 evaluation sessions, the all-firm-day
   conditional coefficient changes sign to +0.00583 [−0.00426,+0.01593]
   (BH q=0.515), while the n≥2 estimate is +0.00295
   [−0.00990,+0.01581] (q=0.653). The nine-rule family has zero BH survivors;
   negative share has IC −0.00312 (p=0.300). The separately predeclared
   evaluation-minus-development contrast is +0.01414
   [+0.00250,+0.02579] (p=0.0173). The approximate pre-run power to recover
   the development effect at the first BH-2 hurdle was only 35.4%, but the
   contrast directly supports a change across eras. The dissertation result is
   therefore temporal non-replication, with no evaluation retuning or trading.
   ...
   entry threshold: risk-off occurs on 0/167 sessions. The recent overlay is
   therefore exactly identical to HAR (net Sharpe 0.240 for both), and all
   recent-window gates fail. Notebook 73's separately frozen backward extension
   reaches risk-off on 53/456 sessions and passes its downside and circular-
   timing tests, but not its useful-risk or full cross-source gate. This is
   partial historical downside timing, not a reason to lower the threshold.
   ...
   opened window.
4. **Gate F1**: completed 2026-08-14. The researcher selected the aggregation/
   temporal-stability RQ with no separate secondary and authorised the one-shot
   Notebook 75 opening. The evidence boundary is recorded in
   `outputs/02_filters_and_distribution/decision.md`.
5. **Promotion**: active. Register Notebook 75 and export only aggregate,
   licence-safe tables/figures needed to write the temporal non-replication.
...
| `36_lseg_gemma_finbert_hybrid_alpha_audit.ipynb` | Audits the single capped Gemma-timing/FinBERT-ranking hybrid against cash with five-session block inference, static market/sector-spanning HAC controls, costs, chronological halves, and rebuilt company/sector exclusions. | Executed; retrospective cash gate passes and stricter factor-alpha gate fails. It supplied the frozen prospective contract, but Notebook 67 later rejects earlier-time historical robustness. |
...
| `44_strategy_evidence_synthesis.ipynb` | Aggregate-only evidence registry, conditional aggregation result, paired sentiment/construction scorecard, cross-regime stress table, implementation/prospective/comparator/lineage evidence, and post-result hybrid controls under an ordinal policy that prevents retrospective Sharpe from overriding independence, stability, and inference. | Refreshed through N73 with 28 registry rows and 20 paired comparisons. Notebook 71 directly supports the bounded FNSPID beyond-mean claim but not LSEG coefficient portability; Notebooks 72–73 add the recent zero-activation and backward partial-downside pressure-transfer rows. One favourable return interval remains Notebook 47's retrospective construction comparison; no AUM, sentiment, validation, or deployment gate is promoted. |
...
| `57_aggregate_candidate_base_frontier.ipynb` | Aggregate-only candidate-role, gate, fixed-cost, and Y=1 impact-capacity audit across already-frozen research and strategy artifacts; it constructs no signal or return and cannot reselect the prospective contract. | Executed; Notebook 36 remains the sole frozen prospective contract, but its factor interval crosses zero, impact Sharpe falls to 0.413 at $10m and −0.632 at $25m, and Notebook 67 later adds a negative earlier-time prior. No validation or deployment gate is promoted. |
...
| `66_lseg_gemma_finbert_backward_replication.ipynb` | Return-blind acquisition and methodology gate for the earlier same-33 LSEG arm, with a company-by-date matrix that distinguishes fully scored, partially scored, downloaded/unscored, and incomplete cells across the backward, existing recent, and prospective collection boundaries; it also retains square-cell backward-frontier detail, complete-date/frontier coverage, per-date request density, storage/capacity projections, immutable-source checks, the frozen endpoint-drift design, signal-population stop, and predeclared one-shot gross/net and mechanism methodology. | Collection complete at 21,912/21,912 company-days and 53,904 pages. The final immutable snapshot has 1,219,608 scorable hashes (`68b98709…`). The 1,774-row frozen Gemma drift audit achieved 0.9892 Spearman, 0.01654 mean absolute difference, 0.9899 direction agreement, zero endpoint sign reversals, and 0.002 control-to-endpoint rate, but exact endpoint retention was 0.7881 against the predeclared 0.80 floor. The conjunctive gate fails permanently; later measurement-only scoring and post-stop sensitivity work do not turn Notebook 66 into a passed replication. |
| `67_lseg_gemma_finbert_joined_long_window.ipynb` | Joins the fully scored backward same-33 population with the original-33 associations from the recent 44-company corpus, reproduces Notebook 36's recent firm-open inputs exactly, and replays its unchanged Gemma-timing/FinBERT-ranking hybrid with frozen costs, cash, factors, halves, circular timing shifts, conditional random names, and rebuilt company/sector exclusions. | Executed post-drift-stop sensitivity. The join has 1,967,554 unique normalized headlines and 16,444 cross-block overlaps. Eleven incomplete backward sessions are forced to cash. Backward gross/net Sharpe is 0.691/−0.354 with −3.43% net return and 6.60-bps/side break-even; cash, factor, stability, dependence, and mechanism gates fail. The 623-session joined path is positive (1.639/0.467 gross/net Sharpe; +6.51%) only because it includes the already-opened recent block. Historical robustness is rejected; the drift failure and prospective claim boundary remain binding. |
...
| `73_lseg_negative_pressure_har_backward_transfer.ipynb` | Applies the exact Notebook 72 state and frozen FNSPID HAR coefficients to the earlier Reuters/LSEG block after a pre-return freeze and hash-manifested `.SP500` LSEG price extension. | Executed bounded historical robustness. Risk-off occurs on 53/456 sessions across 18 entries. Overlay-minus-HAR return is +0.144 bps/session [−1.295,+1.688]; downside BH q=0.0336 and circular downside-timing q=0.0482 pass, but matched-constant attribution, return stability, drawdown magnitude, useful-risk and full cross-source gates fail. No tuning is permitted. |
| `74_conditional_and_pressure_robustness_diagnostics.ipynb` | Labelled post-hoc diagnostics on already-opened data for Notebooks 71 and 73: yearly and HAC-lag stability plus multi-story strata for the conditional coefficient; episode attribution, leave-one-episode-out, and block-length sensitivity for the backward downside result. | Executed. The 71 coefficient is negative in 8/9 years and lag-insensitive but weakens to −0.00529 [−0.01272,+0.00214] on n≥2 firm-days; the 73 downside reduction is spread across episodes (largest 33.2%), survives all 18 leave-one-episode-out drops, and holds at block lengths 10/20/40. Informs phrasing only; no frozen result changes. |
| `75_fnspid_evaluation_beyond_mean_replication.ipynb` | One-shot opening of the FNSPID 2020–2023 evaluation slice for the selected RQ-A estimands: nine-rule IC family (Notebook 03 parameters, BH-9), the all-day and n≥2 conditional coefficients (BH-2), and a separate predeclared evaluation-minus-development stability contrast. | **Executed exactly once after Gate F1.** Zero of nine IC rules survive BH. All-day beta +0.00583 [−0.00426,+0.01593], q=0.515; n≥2 +0.00295 [−0.00990,+0.01581], q=0.653. Evaluation minus development is +0.01414 [+0.00250,+0.02579], p=0.0173. This is temporal non-replication/sign instability; no retuning or evaluation trading followed. |
| `76_fnspid_development_negative_share_double_sort.ipynb` | Development-only model-free dependent double sort: within-day mean-sentiment quintiles, then tie-aware negative-share terciles inside each quintile, with high-minus-low next-open spreads reported in bps. | Executed descriptive translation. The pooled all-firm-day spread is −0.618 bps/session with HAC(5) 95% CI [−1.510,+0.275]; the n≥2 sensitivity is −0.253 [−1.821,+1.315]. The sign is consistent with Notebook 71, but neither interval excludes zero; this is an economic-magnitude figure, not independent confirmation. |
| `77_null_minimum_detectable_effects.ipynb` | Converts existing HAC/bootstrap standard errors into nominal 80%-power two-sided MDEs and conservative first-BH-threshold MDEs for the principal null families. Notebook 10's learned arms are exactly reproduced, then compared with the historical fixed band using a paired block bootstrap. | Executed precision audit. The LSEG conditional blocks could detect only effects about 7.6× and 13.6× the absolute FNSPID coefficient at nominal 80% power. Median nominal MDEs are 18.80 bps/unit for story type, 0.049 IC for publisher, 15.52 bps for earnings interactions, and 6.88 net bps/session for learned-minus-fixed thresholds. MDEs quantify precision; they do not prove zero. |
| `78_null_results_and_provenance_tables.ipynb` | Aggregate-only extraction of declared BH/FDR families plus a core reported-number provenance ledger carrying notebooks, artifacts, full hashes, manifests, frozen specs, seeds and status boundaries. | Executed before Notebook 75. Its historical ledger contains 56 families and 264 saved decisions through Notebook 74: 27 survivors, while 36 families have none. Notebook 75 is registered separately rather than silently inserted into that already-generated table. Promoted LaTeX tables live under `dissertation/tables/`. |
| `79_fnspid_development_reversal_confound.ipynb` | Development-only same-sample conditional rank regression adding the firm's lagged one-session return, lagged five-session return, and trailing 20-session volatility. All controls end at formation open `t`; the outcome runs from `t` to `t+1`. | Executed frozen retrospective robustness. The 511,941-row same-sample baseline is −0.00830 [−0.01410,−0.00251]; the fully controlled coefficient is −0.00914 [−0.01466,−0.00362], p=0.00118 across 2,264 sessions. The primary gate passes; this rejects the simple reversal/lagged-momentum explanation in development, not every omitted variable or a causal null. Notebook 75 later records out-of-era non-replication. |
| `80_lseg33_economic_value_rule_search.ipynb` | Post-hoc, development-only search across 6,672 causal market-wide LSEG-33 warning rules, followed by one unchanged later-block test against HAR and a development exposure-matched constant. | Executed; broad retuning does not repair recent transfer. The selected headline-weighted negative-share 252-session hysteresis rule earns +1.168% versus HAR +1.241% over the later 167 sessions and ends about £729 per £1m behind. It is materially worse on downside and expected shortfall than simply keeping the same lower average exposure. Both economic-value gates fail. |
| `81_fnspid_economic_value_rule_search.ipynb` | Exploratory background search across 1,668 causal market-wide FNSPID negative-news rules, selected on 2013–2019 and applied unchanged to the already-opened 2020–2023 block. | Executed; the selected rule avoids about £187,098 of down-day losses per £1m, but gives up about £201,819 of up-day gains and adds about £5,563 of trading cost. It ends about £21,922 behind HAR and £1,446 behind the development exposure-matched constant. Tail loss improves versus HAR, but utility and matched-exposure attribution fail. |
| `82_lseg33_firm_level_loss_control.ipynb` | Post-hoc, development-only 4,608-rule search for company-specific weight reduction on the fixed 33-stock LSEG panel, with later evaluation against stock-by-stock and aggregate fixed-exposure controls. | Executed; company-specific retuning also fails. The selected Gemma attention-weighted negative-share rule reduces fully invested basket risk, but ends about £30,870 per £1m behind the stock-by-stock matched control, has worse downside/ES, negative results in the first half and every leave-one-company-out replay, and fails every economic-value gate. |
| `83_lseg_prompt_economic_value.ipynb` | User-authorised DeepInfra/Gemma prompt-engineering search over five fixed target-specific Reuters scoring prompts and a strict same-33 company-session population, with 2024 selection, fixed costs and same-date existing-scorer comparators. | Executed; $2.97091685 reported cost. All five prompt portfolios have negative 2024 gross and net means; delayed-reaction h5 is least negative at −0.99/−4.01 gross/net bps per session and −9.96% net ending return. Zero prompts pass, so no 2025 prompt portfolio is computed. The original 99% five-way quality gate and the first-execution test-boundary incident are both preserved; the corrected final run parses zero 2026 prices and materialises zero 2025 event returns. |
| `84_lseg_structured_reaction_score.ipynb` | Direct structured −1 to +1 sentiment score for the anticipated first tradable stock reaction, using the current Reuters headline and target company only; no novelty assessment, prior headline or percentage-return forecast. | Executed post-Notebook-83 sensitivity. Strict 2024 scoring is 3,241/3,242 for $0.18951202. Gross/net means are −2.59/−19.67 bps/session; net Sharpe is −2.18 and ending return −40.53%. It is positive and beats same-date Gemma/FinBERT in only one quarter. Selection fails; 2025 and 2026 stay unscored and no 2025 event return is materialised. |
| `85_fnspid_novelty_conditioned_information.ipynb` | Splits each development firm-day into new versus already-known stories using the strictly-earlier repetition screen, and runs a three-test BH family: mean-sentiment contrast, negative-share contrast, and the already-known share added to the frozen regressors. Includes measurement, threshold sensitivity, marginal models, story-count strata and per-year contrasts. | Executed once on development only; the evaluation block is not loaded. T1 mean contrast is null and wrong-signed (−0.0152, p=0.226). T2 negative-share contrast is −0.0422 [−0.0821,−0.0024] with declared sign but BH q=0.0565; its marginal legs are −0.0234 (p=0.0090) new and +0.0029 (p=0.795) already-known. T3 already-known share is −0.00882 [−0.0139,−0.0038], BH q=0.0019 — the sole survivor, stable at Jaccard 0.40–0.60 and at n≥2, imprecise at n≥5. Rolling contrasts drift toward zero after 2016. |
| `86_fnspid_already_known_share_economic_value.ipynb` | Prices Notebook 85's surviving T3 coefficient: within-session quintile magnitude, four mechanism checks (repeat polarity, Notebook 79 price-path controls, exact-repeat-only, stability), a dollar-neutral book in both raw and conditional-residual form with a cost sweep, a five-condition alpha gate written before the evaluation block is touched, and one unchanged retrospective replay on the already-opened 2020–2023 block. | Executed; **not alpha**, 1/5 gate conditions. Q5−Q1 is −1.93 bps/session [−3.83,−0.10] against a 20-bps round trip and only the top quintile moves. Break-even is 0.96 (raw) and 0.49 (residual) bps/side versus 10 charged, so net Sharpe is −9.75/−13.65. The coefficient survives price controls (−0.00774, p=0.0024) and sits in repeated **non-negative** stories (−0.00962, p=0.00049), not repeated negative ones (p=0.810). The evaluation replay flips the coefficient to +0.00337 (p=0.408) and is negative gross. |
| `frozen_specs/already_known_share_economic_value_v1_20260814.json` | Pre-execution contract for Notebook 86's magnitude translation, four mechanism checks, two books, cost sweep, five-condition alpha gate, and the evaluation-block disclosure that 2020–2023 is already open and the replay is exploratory rather than confirmation. | Complete; gate fails 4/5 and the replay does not reproduce the sign |
| `lib/novelty_conditioning.py` | New/already-known firm-day split that must recombine to the published aggregates, a generic daily cross-sectional rank regression with a reconciling exclusion audit, and the within-session rank residual that makes a conditional coefficient tradable. | Active; eight focused tests cover the threshold boundary, leg recombination, empty legs, a recovered contrast, rank-deficient drops and residual orthogonality |
| `frozen_specs/novelty_conditioned_information_v1_20260814.json` | Pre-execution contract for Notebook 85's novelty definition, return-blind threshold choice, three-test family, inference, strata and sealed-evaluation boundary. | Complete; one BH survivor (T3) on development, recorded as conditioning evidence rather than a validated effect |
| `frozen_specs/examiner_facing_analysis_pack_v1_20260812.json` | Display, precision and provenance choices declared before Notebooks 76–78 ran on already-opened aggregates; records the stopped absolute-bucket double-sort design and its tie-aware dependent-tercile replacement before any spread estimate was computed. | Complete; retrospective packaging contract, not preregistration or confirmation |
...
| `frozen_specs/conditional_negative_share_price_path_controls_v1_20260812.json` | Pre-execution contract for Notebook 79's price-control timing, same-sample comparison, single primary coefficient, inference and no-retuning boundary. | Complete; frozen reversal-confound gate passes |
| `frozen_specs/fnspid_evaluation_beyond_mean_replication_v1_20260812.json` plus `fnspid_evaluation_beyond_mean_preexecution_amendment_v1_20260814.json` | One-shot contract for Notebook 75's eleven declared evaluation tests; the amendment adds outcome-blind power context, a separate HAC(5) stability contrast, exact design/implementation provenance, and a fail-closed empty-output guard without changing the parent families. | Complete; executed once after Gate F1. Both conditional members and the nine-rule family fail to replicate; the stability contrast supports an era shift. |
...
| `frozen_specs/lseg_negative_pressure_har_backward_transfer_v1_20260812.json` | Pre-return contract for Notebook 73's historical `.SP500` acquisition, unchanged pressure/HAR rule, controls, inference, gates and no-retuning boundary. | Complete; partial downside timing, full cross-source support fails |
| `frozen_specs/lseg33_economic_value_rule_search_v1_20260812.json` | Pre-search contract for Notebook 80's 6,672 market-pressure rules, development folds, one-shot later test, matched-exposure attribution, inference and claim boundary. | Complete; recent economic-value gates fail |
| `frozen_specs/fnspid_economic_value_rule_search_v1_20260812.json` | Pre-search contract for Notebook 81's 1,668 market-pressure rules, 2013–2019 selection folds, unchanged 2020–2023 application, matched-exposure attribution, inference and claim boundary. | Complete; exploratory background economic-value gates fail |
| `frozen_specs/lseg33_firm_level_loss_control_search_v1_20260812.json` | Pre-search contract for Notebook 82's 4,608 company-level weight-reduction rules, stock-by-stock fixed exposure control, one-shot later test, 12-test family and stability gates. | Complete; every economic-value gate fails |
| `frozen_specs/lseg_prompt_economic_value_v1_20260813.json` plus authorisation, operational amendments and postflight credit audit | Pre-call contract for Notebook 83's five prompts, payload/privacy route, spend, scoring, 2024 selection, 2025 conditional test, costs, comparators and claim limits. | Complete; append-only scoring cost is $2.97091685. The account-level usage delta is $3.00574212 with $18.529134759 remaining; its $0.03482527 excess is not attributed because the account audit can include other activity or reconciliation. No prompt passes 2024 selection. |
| `frozen_specs/lseg_prompt_economic_value_quality_gate_v2_20260813.json` plus test-boundary incident | Return-blind record of the original five-way 99% NO-GO, the unchanged 98.69% strict-intersection v2, and the first execution's premature 2025 event-return materialisation. | Complete; protocol deviations disclosed, corrected final run materialises no 2025 event return and no 2026 return |
| `frozen_specs/lseg_structured_reaction_score_v1_20260813.json` plus operational, chronological and postflight records | Post-Notebook-83 contract for the researcher's requested structured reaction score, current-headline-only payload, strict score grid, 2024-first hosted scoring, conditional 2025 economics and 2026 seal. | Complete; 2024 gate fails before and after costs. Append-only cost $0.18951202; 2025/2026 scores and 2025 returns remain unopened. |
...
| `frozen_specs/lseg_target_materiality_model_only_development_waiver_20260812.json` plus reaction/comparator amendments | Record the exact researcher waiver, immutable inputs, fail-closed 2026 boundary, eight-test primary family, post-primary reaction-magnitude family, and outcome-selected h5 comparator disclosure. | Complete model-only development analysis. Does not pass the human gate or unlock 2026 confirmation. |
| `lib/joined_lseg.py` | Validates the five immutable scorer artifacts and manifests, restricts the later corpus to original-33 company associations, pairs scorer rows, and performs deterministic cross-block normalized-hash deduplication without exposing headline text. | Active; joined population SHA-256 `4e301e1a…` |
| `frozen_specs/lseg_gemma_finbert_hybrid_joined_long_window_v1.json` + `frozen_specs/lseg_gemma_finbert_hybrid_joined_long_window_completeness_amendment_20260812.json` | Pre-return post-drift-stop analysis contract plus the return-blind rule for 11 incomplete sessions. | Complete; incomplete sessions remain in the calendar as cash and cannot be dropped, imputed, carried, or shifted |
| `frozen_specs/lseg_gemma_continuous_joined_long_window_v1_20260812.json` | Pre-execution contract for Notebook 70's unchanged continuous reducer, adaptive schedule, breadth, accounting, costs, two source-block inference family, factors, temporal gates and no-retuning boundary. | Complete; retrospective temporal no-go, with the separate materiality 2026 confirmation still sealed |
| `lib/incremental_finbert.py` + `../scripts/materialize_lseg_incremental_finbert_snapshot.py` | Validates complete cursor chains and materializes a memory-bounded compact immutable LSEG-style headline snapshot from terminal company-date checkpoints only, without querying LSEG or loading prices/returns. | Third snapshot complete: 17,967 company-dates, 42,899 included pages, 884,863 unique scorable headlines, population SHA-256 `a9349fdd…`; all 607,097 prior hashes are retained, zero are dropped, and 277,766 new hashes are isolated; snapshots and licensed text remain ignored/local |
| `frozen_specs/lseg_gemma_finbert_hybrid_backward_incremental_scoring_amendment_20260808.json` | Researcher-authorised deadline amendment permitting incremental pinned local FinBERT preprocessing while preserving the final-population reconciliation and every price/return/strategy boundary. | Active; Gemma remains on the existing DeepInfra FP8 API contract |
| `frozen_specs/lseg_gemma_finbert_hybrid_backward_incremental_gemma_scoring_amendment_20260809.json` | Researcher-authorised paid-scoring amendment binding the 607,097-headline second snapshot to the unchanged Gemma 4 26B / DeepInfra FP8 / investor-prompt / ZDR contract, append-only hash reconciliation, and a 300,000-attempt first tranche. | First tranche complete: 299,987 unique successes, 13 preserved API errors, $11.30746476 total reported cost, zero duplicate successes, invalid successful probabilities, or route mismatches, and output SHA-256 `b85879da…`; 307,110 snapshot hashes remain, while prices, returns, partial strategy inference, and retuning remain sealed |
| `frozen_specs/lseg_gemma_finbert_hybrid_backward_incremental_gemma_tranche2_20260809.json` | Explicit approval record for a second 200,000-attempt tranche on the unchanged append-only snapshot/checkpoint and OpenRouter → DeepInfra FP8/ZDR contract. | Complete: 199,998 successes plus two API errors from 200,000 attempts for $7.53824224; cumulative checkpoint 499,985 unique successes, 15 preserved errors, 107,112 unresolved hashes, zero validation failures, output SHA-256 `2bd88cef…` |
| `frozen_specs/lseg_gemma_finbert_hybrid_backward_incremental_gemma_final_tranche_20260810.json` | Explicit approval record for attempting all 107,112 unresolved hashes on the unchanged append-only snapshot and Gemma/DeepInfra FP8/ZDR contract. | Stopped with its remaining 1,968-hash tail preserved for the separately authorised lower-concurrency cleanup; cumulative checkpoint before cleanup had 605,129 unique successes and $22.80902699 reported cost |
| `frozen_specs/lseg_gemma_finbert_hybrid_backward_incremental_gemma_cleanup_20260810.json` | Explicit approval record for retrying the 1,968 unresolved hashes after the concurrency-25 tail stalled, without changing the scorer or privacy contract. | Complete-partial: 1,944 new successes from 1,968 attempts; final cumulative coverage 607,073/607,097 (99.9960%), 24 unresolved, $22.88230343 reported cost, zero duplicate successes, invalid successful probability vectors, route mismatches, or checksum failures; output SHA-256 `589ddbe8…` |
| `frozen_specs/lseg_gemma_finbert_hybrid_backward_second_account_incremental_scoring_20260810.json` | Explicit approval and spend gate for all outstanding FinBERT and Gemma hashes in the 884,863-headline second-account snapshot, retaining the scorer/privacy contracts and sealing returns. | Gemma running on 277,790 pending hashes; 277,766 FinBERT hashes are staged and watched for the first reachable UCL GPU |
| `frozen_specs/lseg_gemma_finbert_hybrid_backward_gemma_drift_result_20260811.json` | Aggregate-only result of the predeclared complete-collection Gemma endpoint-drift audit. | Failed stop: 1,774/1,774 successful rescoring requests cost $0.06719615; five gates pass and exact endpoint retention is 0.7881 versus the frozen 0.80 minimum. The original replication remains stopped. Notebook 67's later separately frozen return sensitivity does not rescind or complete it. |
| `frozen_specs/lseg_gemma_finbert_hybrid_backward_full_scoring_override_20260811.json` | Explicit researcher override permitting measurement-only completion of both final-population scorers after the drift stop, without rescinding it or opening returns. | Complete: FinBERT and Gemma both cover 1,219,608/1,219,608 final hashes. Four superseded prior hashes are excluded; Gemma's 21-error tail was fully recovered at concurrency 5 for $12.70683447 incremental reported cost. |
| `../scripts/audit_lseg_backward_pipeline_status.py` | Aggregate-only reconciler for terminal LSEG download coverage, staged/completed FinBERT, and multiple append-only Gemma artifacts, including hashes, route/probability/duplicate checks, reported cost, and remaining population. | Current ignored status uses the 884,863-hash snapshot, reports live Gemma progress across seed plus delta, and records FinBERT's 607,097 reusable/277,766 pending split; it never prints headlines or loads prices/returns |
| `../scripts/materialize_lseg_baseline_resume_seed.py` | Rebuilds reusable exact-hash FinBERT/VADER rows with canonical metadata from the current target snapshot while inheriting probabilities only. | 1,214,194 score rows inherited for 607,097 hashes; 3,193 hashes had metadata canonicalised; seed SHA-256 `c51494ae…` |
| `../scripts/ucl_run_lseg_backward_incremental_finbert.sh` + `../scripts/watch_and_launch_lseg_finbert_ucl.py` | Occupancy-gated UCL tmux launcher plus one-shot five-minute pool watcher, with pinned revision, cache-only inference, immutable snapshot/seed hashes, durable logs, and run record. | Third snapshot and canonical seed are verified on shared storage; initial pool check found 54 offline workers, so the watcher remains active until a safe launch succeeds |
| `lib/backward_validation.py` | Tested fail-closed collection audit, terminal company-date completion ledger, per-date checkpoint/frontier ledger, request and storage projections, deterministic census-plus-control Gemma drift sampling, and endpoint/continuous drift metrics. It imports no price or portfolio loader. | Active; used by Notebook 66 while collection accumulates |
...
| `outputs/73_lseg_negative_pressure_har_backward_transfer/` | Aggregate backward pressure/state audit, HAR/overlay results, inference, timing controls, figures and manifest; no headline text. | Ignored/local; bounded historical robustness result |
| `outputs/74_conditional_and_pressure_robustness_diagnostics/` | Yearly/HAC-lag/strata tables for the conditional coefficient; episode attribution, leave-one-episode-out and block-length tables for the backward pressure result; figures. | Ignored/local; labelled post-hoc diagnostics |
| `outputs/75_fnspid_evaluation_beyond_mean_replication/` | Nine-rule IC family, two-member conditional family, development/evaluation daily coefficient series, prospective power context, predeclared stability contrast, result summary, postflight validity audit and hash manifest. | Ignored/local; completed one-shot chronological evaluation result |
| `outputs/76_fnspid_development_negative_share_double_sort/` | Double-sort cell/spread tables, pooled summaries, input audit, aggregate figure and manifest. | Ignored/local; development-only descriptive result; promoted figure is licence-safe |
| `outputs/77_null_minimum_detectable_effects/` | Test-level and family-level MDE tables, exact threshold-arm reproduction, paired learned-minus-fixed bootstrap audit, LSEG precision figure and manifest. | Ignored/local; arithmetic precision audit; promoted LaTeX table is licence-safe |
| `outputs/78_null_results_and_provenance_tables/` | Full multiplicity-family ledger, summary, core result-provenance ledger, Markdown previews and manifest. | Ignored/local; aggregate audit; promoted LaTeX tables are licence-safe |
| `outputs/79_fnspid_development_reversal_confound/` | Price-control coverage, five declared coefficient rows, full-model coefficient audit, daily coefficients, primary gate, figure, table and hash manifest. | Ignored/local; development-only frozen robustness; promoted figure and LaTeX table are licence-safe |
| `outputs/80_lseg33_economic_value_rule_search/` | Full candidate grid, selected rule, later daily paths, exposure controls, corrected inference, costs, utility, economic decomposition, gates and manifest. | Ignored/local; post-hoc chronological search; no licensed text |
| `outputs/81_fnspid_economic_value_rule_search/` | Full candidate grid, selected rule, evaluation daily paths, matched-exposure control, corrected inference, costs, utility, economic decomposition, gates and manifest. | Ignored/local; exploratory chronological background; no headline text |
| `outputs/82_lseg33_firm_level_loss_control/` | Full candidate grid, selected company-risk rule, later daily paths, three exposure controls, corrected inference, economic decomposition, state/half/company diagnostics, costs, gates and manifest. | Ignored/local; post-hoc chronological search; no licensed text |
...
Gate F1 selected the temporal-stability aggregation question on 2026-08-14, and
Notebook 75 completed its one-shot evaluation opening. The current task is
promotion and dissertation writing around the preserved temporal non-replication
result, not another experiment search.
