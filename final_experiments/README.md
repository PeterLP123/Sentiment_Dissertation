...
3. **Exploratory secondary arms**: story type, earnings, pooled learned thresholds, and all three non-pooled LSEG regimes have now been run. See [`EXPLORATORY_EXPERIMENT_LEDGER.md`](EXPLORATORY_EXPERIMENT_LEDGER.md).
   ...
   through Notebook 56. The sentiment-free HAR target remains the research
...
| `44_strategy_evidence_synthesis.ipynb` | Aggregate-only evidence registry, paired sentiment/construction scorecard, cross-regime continuous-rule stress table, implementation-capacity evidence, prospective population status, historical mid-cap comparator reconciliation, and local strategy-lineage coverage under an ordinal policy that prevents retrospective Sharpe from overriding independence, stability, and inference. | Refreshed through N56 with 20 registry rows; HAR retained only as research benchmark, the positive mid-cap FinBERT result restored as tier-2 historical evidence and verified as the only adequately covered positive local LSEG/FinBERT family, one of eleven favourable paired return intervals excludes zero but is retrospective and non-promoting, the hybrid's optimistic grid capacity is $10m, the prospective ledger remains at 27/120 sessions, and no deployment-qualified base or sentiment strategy exists |
