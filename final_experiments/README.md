...
3. **Exploratory secondary arms**: story type, earnings, pooled learned thresholds, and all three non-pooled LSEG regimes have now been run. See [`EXPLORATORY_EXPERIMENT_LEDGER.md`](EXPLORATORY_EXPERIMENT_LEDGER.md).
   ...
   through Notebook 53. The sentiment-free HAR target remains the research
...
| `44_strategy_evidence_synthesis.ipynb` | Aggregate-only evidence registry, paired sentiment/construction scorecard, cross-regime continuous-rule stress table, implementation-capacity evidence, and prospective population status with an ordinal policy that prevents retrospective Sharpe from overriding independence, stability, and inference. | Refreshed through N53; HAR retained only as research benchmark, one of eleven favourable return intervals excludes zero but is retrospective and non-promoting, the hybrid's optimistic grid capacity is $10m, liquidity sizing stops before returns, the first prospective batch stops at 27/120 sessions, and no deployment-qualified base or sentiment strategy exists |
