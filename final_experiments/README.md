...
| Beyond-mean candidate | Notebook 71 directly tests negative-story share after controlling for mean continuous sentiment and news count. The FNSPID development coefficient is −0.00831 with 95% HAC CI [−0.01410, −0.00252] (p=0.0049), so the bounded core claim passes. Notebook 79 then fixes the reversal-confound specification before execution and adds the firm's lagged one-session return, lagged five-session return, and trailing 20-session volatility on the same price-complete development sample. The coefficient strengthens to −0.00914 [−0.01466, −0.00362] (p=0.00118), so the simple price-path explanation does not account for the development result. Both primary LSEG coefficients have the same negative sign but intervals cross zero; portability does not pass. |
...
3. **Exploratory secondary arms**: story type, earnings, pooled learned thresholds, and all three non-pooled LSEG regimes have now been run. See [`EXPLORATORY_EXPERIMENT_LEDGER.md`](EXPLORATORY_EXPERIMENT_LEDGER.md).
   ...
   robustness, not causality or independent confirmation, and Notebook 75
   remains sealed.
...
| `75_fnspid_evaluation_beyond_mean_replication.ipynb` | One-shot opening of the FNSPID 2020–2023 evaluation slice for the RQ-A estimands: nine-rule IC family (Notebook 03 parameters, BH-9) and the conditional negative-share coefficient with a pre-declared n≥2 stratum (BH-2). | **Prepared, deliberately unexecuted.** Design frozen in `frozen_specs/fnspid_evaluation_beyond_mean_replication_v1_20260812.json`; input gate verified. Execute once, only after confirming the Gate F1 RQ-A framing. |
...
The LSEG economic-value chain now includes executed Notebooks 80 and 82; Notebook 75 remains deliberately sealed. Gate F1 and promotion remain decisions,
