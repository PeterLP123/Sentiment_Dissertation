"""Shared evaluation helpers for final_experiments arms.
...
def build_daily_portfolio(
    ...
    *,
    ...
    Returns one row per session with gross/net return and turnover.
    """
    cfg = config or TradeConfig()
    need = {"session_date", "symbol", signal_col, cfg.outcome}
    ...
    frame = frame.dropna(subset=[signal_col, cfg.outcome]).copy()
    if frame.empty:
        return pd.DataFrame(
            columns=[
                "session_date",
    ...
    for session, day in frame.groupby("session_date", sort=True):
        ...
        # One-sided turnover against yesterday's book. Names absent from either
        # side count once via ``.get(s, 0.0)``.
        names = set(prev_weights) | set(weight_map)
        turnover = 0.5 * sum(
            abs(weight_map.get(s, 0.0) - prev_weights.get(s, 0.0)) for s in names
        )
