...
| Current active work | Decide the Gate-F1 dissertation framing from the completed evidence. Notebook 71 now supplies the missing direct RQ-A test: after controlling for mean sentiment and news count, the FNSPID development negative-share coefficient is −0.00831 [−0.01410,−0.00252] (p=0.0049). Both primary LSEG coefficients point negative but remain imprecise, and their costed books lose. Notebook 72's exact FNSPID pressure-state transfer never enters risk-off on the 167-session LSEG window. This makes the bounded “distribution beyond the mean in FNSPID, with limited Reuters portability” framing the strongest current candidate, not a validated strategy or universal claim. The materiality measurement/h5 results remain secondary and the 2026 confirmation remains sealed. | Stage S2 exit / Gate F1; Notebooks 40–72 |
...
| Conditional aggregation / cross-source risk transfer | Complete; bounded FNSPID pass, LSEG portability and exact pressure transfer fail | Notebook 71 estimates the direct negative-share-beyond-mean coefficient on FNSPID development and separate pinned-FinBERT LSEG backward/recent blocks. FNSPID passes; both LSEG intervals cross zero. Notebook 72 uses the unchanged FNSPID pressure-state specification on LSEG, where zero recent sessions reach the risk-off entry threshold. Neither result permits source pooling, threshold tuning or an alpha claim; `final_experiments/71_conditional_negative_share_transfer.ipynb`, `final_experiments/72_lseg_negative_pressure_har_transfer.ipynb` |
...
outcomes. Refreshed through Notebook 72, it assigns an ordinal evidence tier to
27 representative strategy rows, keeps cross-regime Sharpes explicitly
non-comparable, and applies one promotion policy to nineteen paired sentiment or
...
Notebook 71 separately supplies the direct conditional aggregation estimand,
while Notebook 72 records an exact cross-source pressure transfer with zero
state activation.
