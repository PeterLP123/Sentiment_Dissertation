...
| Current active work | Decide the Gate-F1 dissertation framing from the completed evidence. Notebook 68 has frozen the return-blind 2,000-event Reuters parent sample, 1,445-event body-available population and 1,445/1,445 valid DeepInfra/Gemma annotations; the blinded 200/60 human audit remains open. Under the explicit model-only waiver, Notebook 69 completes a 1,200-event pre-2026 development analysis with zero confirmation returns opened. Its eight-test directional and three-test reaction-magnitude families are null after BH. A fixed h5 novelty-weighted materiality translation is descriptively cost-positive but does not beat cash; its favourable FinBERT comparison is outcome-selected post-primary. The viable writing contribution is the separation of semantic richness, implementation economics and predictive alpha—not a validated strategy. Historical strategy expansion remains closed, Notebook 67 still forbids retuning the old hybrid, and 2026 confirmation remains sealed. | Stage S2 exit / Gate F1; Notebooks 40–69 |
| Backward external-time arm | Post-stop sensitivity complete; historical robustness rejected and original drift failure remains binding | Same 33 companies, 2024-01-01 to 2025-10-26; 21,912/21,912 terminal company-days and 1,219,608 unique scored hashes. Notebook 67's unchanged hybrid replay forces 11 incomplete sessions to cash and records backward gross/net Sharpe 0.691/−0.354, −3.43% net return, 6.60-bps/side break-even, zero-crossing cash and sector-factor intervals, failed mechanism tests, and only 1/33 company and 3/11 sector exclusions profitable. The 623-session joined path is positive only after adding the already-opened recent block. The predeclared drift census still fails endpoint retention at 0.7881 versus 0.80, so this cannot be called the original replication or prospective evidence; `final_experiments/67_lseg_gemma_finbert_joined_long_window.ipynb` |
...
outcomes. Refreshed through Notebook 65, it assigns an ordinal evidence tier to
23 representative strategy rows, keeps cross-regime Sharpes explicitly
non-comparable, and applies one promotion policy to fifteen paired sentiment or
...
1. use Notebooks 44–67 to write the cross-regime conclusion: HAR remains the independent price/risk research benchmark but is not time-stable or deployment-qualified; aggregate sentiment shows one-period downside timing; Gemma/LSEG adds mechanical de-risking and gross opened-window mechanisms, but the unchanged hybrid fails its earlier same-33 robustness block at −0.354 net Sharpe and −3.43% net return, with cash, factor, mechanism, stability, and dependence gates failing. Its positive 0.467 joined net Sharpe is a composition of a negative earlier block and the already-opened strong recent block, not temporal robustness. The original Gemma drift failure separately prevents a preregistered-replication claim. Preserve the implementation, attribution, and metadata-filter diagnostics as secondary opened-window evidence, not as a strategy rescue;
...
5. preserve Notebook 67 as the only backward same-33 strategy sensitivity: do not revise the failed drift threshold, hide the negative block inside the joined path, or tune the signal, costs, exclusions, or completeness treatment;
