...
outcomes. Refreshed through Notebook 73, it assigns an ordinal evidence tier to
28 representative strategy rows, keeps cross-regime Sharpes explicitly
non-comparable, and applies one promotion policy to twenty paired sentiment or
...
not a cash-alpha result and does not pass promotion. Seven comparisons report
...
while Notebooks 72–73 show zero recent pressure-state activation and partial
backward downside timing without a full cross-source pass.
