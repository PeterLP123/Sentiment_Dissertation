...
Scores the exact headline population `analyze-headline-value` aggregates (unique in-window company-matched headlines) with any provider, into an append-only CSV keyed by normalized-headline hash. Re-running skips already-successful rows and re-attempts failures; `--limit` takes a hash-ordered (effectively random) subsample for unbiased pilots. `--source-code` applies an exact repeatable source filter, `--direct-company-only` keeps only single-company actionable headlines, `--max-population` refuses an unexpectedly large final population, and `--dry-run` reports that frozen population without opening a model client. Pass the same `--source-code` and `--direct-company-only` options to `analyze-headline-value` so LLM and baseline scorers use one frozen population.

`label_only` prompts score +1/0/-1. `soft_label` prompts persist `p_positive`, `p_negative`, and `p_neutral`, plus `score = p_positive - p_negative` in [-1, 1] and `score_100 = 100 * score` in [-100, 100]. The parser requires all three nonnegative probabilities, an exact key set, a unit sum within tolerance, and bounded scores. The CSV also records normalized label, exact model digest when available, prompt hash, status/error, latency, and token counts/rates. An adjacent manifest hashes the inputs and output and records the model, prompt, inference settings, device, timestamps, runtime, and row reconciliation. Keep separate immutable outputs per prompt or model identity.

For Ollama, `--ollama-think/--no-ollama-think` controls thinking explicitly, `--ollama-keep-alive -1` keeps the model loaded, and `--structured-output` requests the provider's probability JSON schema. Headline scoring honors the explicit completion budget, including a 32-token soft-label budget. If `financial_soft_label_base` receives only headline text, it measures general financial headline tone rather than company-specific impact; do not describe it as a target-company score. Explicit target identity is separately flagged when the headline contains a configured company name/alias or exact RIC.
