# Frozen LSEG Analysis Workflow

Last updated: 2026-06-28

This is the execution contract from the completed LSEG collection to the frozen L2/L3 analyses. The primary design is the 33-company US single-stock panel over `2025-12-26T00:00:00Z` to `2026-06-26T00:00:00Z`. Index futures are a secondary overlay. The design does not make a pre/post-2023 comparison.

The raw collection is resumable and may continue independently. Formal scoring remains blocked until the raw manifest, cleaned corpus, quality validation, and cohort freeze are complete. Raw checkpoints and licensed story text are never source-controlled.
...
flowchart LR
    ...
    V --> F["Frozen prompts, models, tags and digests"]
...
## 4. Freeze And Score The Crossed Matrix

The frozen LLM roster is GPT-4o mini, Gemini 2.5 Flash Lite, Llama 3.3 70B Instruct, `gemma3:12b`, and `qwen3:8b`. FinBERT and VADER are deterministic contrasts. The soft-label prompt families cross target-company/general-financial wording with base, label-order, and semantic-paraphrase variants.
...
The frozen dry-run is 147,500 calls: 100,000 LSEG calls and 47,500 labeled-benchmark calls, split into 88,500 hosted and 59,000 local calls. The full cohort/benchmark tier receives the base prompt; the 500-item facet subsets receive the two additional variants. Five stochastic samples are stored per crossed cell.

Every response identity includes item ID and content hash, model ID and digest, prompt ID and hash, and sample index. Probabilities, label, token usage, latency, cost, parse status, and errors are recorded. Resume skips exact matches only. Formal execution fails if local Ollama tags/digests cannot be verified or if any frozen identity drifts.
...
- A changed cohort rule, prompt, roster, model digest, split, or analysis parameter creates a new run; it does not overwrite the frozen one.
