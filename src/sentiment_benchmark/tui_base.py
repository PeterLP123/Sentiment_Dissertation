...
# Run-size (total request count) above which the TUI asks for confirmation.
_CONFIRM_THRESHOLD = 1000

# Run-tab input ids that carry validators; used to gate Start and persist settings.
...
class AppMixin(_AppBase):
    if TYPE_CHECKING:
        ...
        def _set_busy(
            self, button_id: str | None, busy: bool, *, label: str | None = ..., spinner_widget: str | None = ...
        ) -> None: ...
        def _update_sort_help(
            self, help_id: str, base_text: str, sort_state: tuple[int, bool] | None, columns: dict
        ) -> None: ...
        def _settings_valid(self) -> bool: ...
        def _build_run_config_from_ui(self) -> RunConfig: ...
        async def _execute_config(self, config: RunConfig) -> RunSummary: ...
