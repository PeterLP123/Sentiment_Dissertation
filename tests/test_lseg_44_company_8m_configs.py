...
def test_sector_33_long_backward_replication_matches_frozen_queries() -> None:
    base = load_lseg_collection_config(
        ROOT / "configs/lseg_us_sector_33_prospective_20260626_20260806_headlines.toml"
    )
    backward = load_lseg_collection_config(
        ROOT / "configs/lseg_us_sector_33_backward_20240101_20251026_headlines.toml"
    )

    assert [
        (company.symbol, company.name, company.ric, company.news_query, company.aliases)
        for company in backward.companies
    ] == [
        (company.symbol, company.name, company.ric, company.news_query, company.aliases)
        for company in base.companies
    ]
    assert backward.start == "2024-01-01T00:00:00Z"
    assert backward.end == "2025-10-26T00:00:00Z"
    assert backward.fetch_story_bodies is False
    assert backward.window_days == 1
    assert backward.max_requests_per_run == 9500
    assert all("Source:" not in company.news_query for company in backward.companies)
