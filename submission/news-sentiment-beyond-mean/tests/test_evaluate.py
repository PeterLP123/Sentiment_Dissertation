"""Focused tests for experiments evaluation / turnover helpers."""
...
from experiments.lib.evaluate import (
...
def test_daily_portfolio_turnover_and_breakeven() -> None:
    ...
    # Day 2 flip costs 1.0, then final liquidation costs another 0.5.
    assert abs(daily.loc[1, "turnover"] - 1.5) < 1e-12
...
def test_daily_portfolio_rebalances_from_return_drifted_weights() -> None:
    frame = pd.DataFrame(
        {
            "symbol": ["A", "B", "A", "B"],
            "session_date": pd.to_datetime(
            ...
            "ret_open_h1": [0.10, 0.0, 0.0, 0.0],
        }
    )
    daily = build_daily_portfolio(frame, "signal", split="development")

    # After day 1, A/B drift from +0.5/-0.5 to +11/21/-10/21.
    # Day 2 therefore trades 1/42 before its separate 0.5 final liquidation.
    assert daily.loc[1, "turnover"] - 0.5 == pytest.approx(1.0 / 42.0)


def test_daily_portfolio_charges_terminal_liquidation() -> None:
    frame = pd.DataFrame(
        {
            "symbol": ["A", "B"],
            "session_date": pd.to_datetime(["2020-01-02", "2020-01-02"]),
            "split": ["development"] * 2,
            "signal": [1.0, -1.0],
            "ar_open_h1": [0.0, 0.0],
            "ret_open_h1": [0.0, 0.0],
...
def test_drifted_weight_map_rejects_an_insolvent_book() -> None:
    with pytest.raises(PortfolioInsolvencyError, match="non-positive") as error:
        drifted_weight_map(
            ["A", "B"],
            np.array([0.5, -0.5]),
            np.array([0.0, 3.0]),
