...
boundary produces a different cross-sectional ordering. This dissertation
separates two questions: whether that negative-story share is conditionally
associated with the abnormal return following a firm's news-assigned session
after average sentiment, news volume and recent prices are controlled in
development; and whether the unchanged baseline association, conditional on
mean sentiment and news volume, persists in 2020--2023. The panel is sparse:
48.3\% of the 512,153 development firm-days carry one story and the median
carries two.
...
Negative-story share alone survives correction in development. The baseline
coefficient is $-0.00831$, with a 95\% interval of
$[-0.01410,-0.00252]$; adding one-session, five-session and volatility controls
gives $-0.00914$, with interval $[-0.01466,-0.00362]$. The frozen evaluation
repeats the baseline specification and gives $+0.00583$, with interval
$[-0.00426,+0.01593]$ and BH $q=0.515$; the multi-story coefficient and the
nine-rule family also fail. The baseline evaluation-minus-development contrast
is $+0.01414$, with interval $[+0.00250,+0.02579]$ and $p=0.0173$. Prospective
power to recover the development effect at the first BH-2 hurdle was only
35.4\%, so the evaluation null is not proof of zero. The contrast establishes
instability in the measured baseline pipeline association, not its cause.
...
Financial news rarely arrives as one observation per firm and trading day. An
earnings alert, results story, analyst reaction and later updates must be
compressed into one firm-day value. The arithmetic mean is convenient, but it
can hide how many stories cross a negative decision boundary. Two days can
therefore share a similar mean while receiving different negative-story shares.
The aggregation rule determines which part of the observed flow enters the
return model.
...
The study separates a development question from a temporal-validation question:

\begin{quote}
\textbf{Development:} Is the share of negative stories in a firm's assigned-session news
flow conditionally associated with the following
abnormal-return rank after controlling for mean sentiment, news volume and
recent price dynamics?

\textbf{Temporal validation:} Does the unchanged baseline association,
conditional on mean sentiment and news volume, remain negative in the frozen
2020--2023 FNSPID evaluation?
\end{quote}
...
\begin{enumerate}[label=H\arabic*.,leftmargin=1.1cm]
  ...
  \item[H1b.] The unchanged baseline all-firm-day and multi-story coefficients remain negative in the frozen 2020--2023 replication under their declared correction.
...
mean, coverage and recent-price controls. Because single-story days dominate,
this is principally a measurement comparison. Second, it preserves a frozen
temporal failure: the baseline development association changes sign in
2020--2023, and a predeclared contrast tests that change directly. Third, it
translates the result into return scale, costs and separate-source tests while
retaining adverse economic and portability evidence. The contribution is the
evidence sequence, not a universal sentiment signal.
...
Temporal transport is a separate standard from development significance.
\citet{welch2008} find that many proposed equity-premium predictors are unstable
and perform poorly out of sample. Across 97 cross-sectional return predictors,
\citet{mclean2016} report substantial attenuation outside the original sample
and after publication. \citet{farmer2023} show that benchmark return
predictability can be local in time, with short predictive pockets separated by
long inactive periods. These studies motivate the frozen era comparison here.
They also limit its interpretation: a changed coefficient establishes that the
measured predictor-pipeline association did not transport, not whether markets,
sample composition or measurement caused the change.
...
Other limitations are unequal source density, classifier error and survivorship
in the coherent priced cohort. The panel includes only firm-days with news, a
valid symbol, a stock open and a SPY open; the 33 LSEG firms are large and
heavily covered. The aggregate snapshot also contains no common-universe
re-estimate or per-era decomposition of firm coverage, story mix, class balance
and timestamp precision. The FNSPID era contrast therefore identifies
instability in the measured pipeline association. It cannot distinguish a
market change from shifts in sample composition, news production, mapping or
...
tests whether the coefficients from \cref{eq:conditional-base} differ; the
price-path specification in \cref{eq:conditional-full} is not part of this
contrast. It avoids treating significance in one era and non-significance in
another as sufficient evidence of a change, and cannot rescue a failed primary
family.
...
$[-0.00990,+0.01581]$ and $q=0.653$. H1b is not supported under its recorded
...
temporal non-replication and instability in the measured baseline pipeline
association. The evidence does not identify its cause, and it does not
establish equivalence or reversal as a new trading rule.
...
The oriented negative-share portfolio grosses $0.890$ basis points per session
and has one-way turnover of $0.712$, so 10 basis points per side implies a
14.23-basis-point charge and a $-13.34$-basis-point net mean. Its break-even
cost is $0.890/(2\times0.712)=0.625$ basis points per side. Across all nine
rules, break-even costs range from $0.116$ to $0.625$ basis points, making the
charged cost 16.0 times the best value in \cref{fig:break-even}.
...
Learned filters do not rescue the rule: their evaluation AUCs range from
$0.5005$ to $0.5038$, and they lose $7.71$ to $16.88$ additional basis points
per session against the fixed band. H3 therefore fails for the tested
implementation. Turnover overwhelms the gross spread.

\section{FNSPID risk timing appears only within a narrow boundary}

Over 998 sessions in 2020--2023, adding the negative-pressure rule to HAR
raises net return from 29.85\% to 35.20\% while reducing mean exposure from
$0.619$ to $0.569$. The paired mean advantage is imprecise at $0.366$ basis
points per session (95\% interval $[-0.996,1.930]$), but squared downside falls
with $p=0.0148$. The overlay gains $2.731$ basis points per session in 2020 and
loses $0.437$ over 2021--2023; its 2013--2019 return difference is also
imprecise. \Cref{fig:risk-regime} shows this concentration.

Only about 1\% of non-zero circular shifts match the 2020--2023 downside
reduction, giving BH $q=0.0301$ across the three regimes in
\cref{fig:downside-placebo}. The 2013--2019 and LSEG/Gemma schedules do not
survive correction, and no return-timing test survives. The useful-risk gate
therefore fails: one period contains downside-timing evidence, not a stable
deployment rule.
...
An earlier 44-firm LSEG comparison produced no BH survivor across either an
18-test aggregator/scorer family or an eight-test scorer/filter family. It is
a different cohort and does not settle transfer here, but prevents the two
imprecise coefficients from being treated as the only LSEG evidence. Their
80\%-power MDEs are $0.0632$ and $0.1126$, respectively 7.6 and 13.6 times the
FNSPID baseline coefficient. \Cref{fig:cross-source-power} therefore supports
insufficient precision, not equivalence to zero.
...
Gemma is a secondary sensitivity: its backward coefficient is $-0.05443$
($p=0.0156$), but the recent estimate is imprecise and the economic gates fail.
The exact FNSPID risk rule never activates in recent LSEG because its maximum
$z$-score, 1.349, is below the fixed 1.5 threshold. In the backward block it is
active for 53 of 456 sessions. Downside and timing tests survive their declared
corrections, but the return difference is $0.144$ basis points with interval
$[-1.295,1.688]$, and two of eighteen episodes supply 65.6\% of the downside
reduction. Leave-one-episode-out and block-length checks survive, so no single
episode explains the result; the concentration and recent non-activation still
prevent an operating-rule claim.

\section{Tuned LSEG rules do not create incremental economic value}

Allowing the LSEG rule to change does not help. The 6,672-candidate aggregate
search produces a $-0.049$-basis-point paired evaluation difference, with
interval $[-0.656,0.633]$; matched downside metrics also favour the control.
The firm-level news rule reduces loss relative to an always-invested portfolio
but not relative to development-selected constant company weights. Against
that matched control it has 0.63 percentage points more drawdown, 0.27 points
more downside deviation and 25.9 times the turnover. Its paired return
difference is $-1.743$ basis points, with interval $[-4.727,1.549]$.
...
\Cref{fig:lseg-matched-value} shows the economic reason: per \$1 million, the
rule avoids \$24,738 of losses but gives up \$41,126 of gains, adds \$12,715 of
cost and ends \$30,870 behind. Both halves and leave-one-company-out checks are
reported in the aggregate evidence; every complete economic-value gate fails.

\section{Prompt engineering does not improve the LSEG result}

All five target-specific prompts fail their 2024 selection gate. The best is
already negative before costs ($-0.989$ basis points per session) and loses
$-4.012$ after costs. The structured reaction score is worse: its gross mean is
$-2.590$ basis points and its net mean $-19.666$. The stopping rule therefore
prevents a 2025 portfolio from being computed.
...
\Cref{fig:prompt-gate} shows all six failures. They reject the tested prompt
constructions, not every possible instruction; persuasive generated rationales
are not evidence of return-ranking ability.

\section{Nulls and the final answer}

The frozen audit through Notebook 73 contains 56 families and 264 tests or
decisions: 27 within-family BH survivors and 36 all-null families. It includes
the timing families but predates the focal conditional, final economic and
prompt studies, which remain separately recorded. The totals expose selection
pressure; heterogeneous estimands and dates prevent a project-wide
...
The exploratory development estimate is consistent with H1a under the selected
specification, but it is not post-selection-adjusted confirmation. H1b is not
supported. H2 has the predicted direction but an imprecise spread; H3 fails on
costs; H4a remains unresolved because LSEG is imprecise; and H4b fails every
complete matched-control gate. The finding motivates prospective measurement
work, not a rich-news distribution, trading or risk-management claim.
````
...
The development and temporal questions have different answers. Through 2019,
negative-story share is conditionally associated with the following
abnormal-return rank beyond rank-linear mean sentiment and news volume: the
baseline coefficient is $-0.00831$, with a 95\% HAC interval of
$[-0.01410,-0.00252]$. Adding recent-price controls gives $-0.00914$, with
interval $[-0.01466,-0.00362]$. The frozen 2020--2023 opening repeats the
baseline model, not the price-control model, and gives $+0.00583$, with interval
$[-0.00426,+0.01593]$ and BH $q=0.515$; the multi-story coefficient and
nine-rule family also fail.

The direct baseline era comparison matters more than contrasting two
significance labels. Evaluation minus development is $+0.01414$, with interval
...
families and direct contrast establish non-replication of the measured baseline
pipeline association. They do not identify why it changed.
...
outcome is preserved, and a predeclared HAC contrast tests the across-era
coefficient difference directly. Rank translation, costs, LSEG transfer and
matched comparators then show why development detectability is not economic or
portable validity.
...
than verified labels. Mean $p(\mathrm{negative})$ keeps the next-open sign but
does not survive correction; close-to-close and FF3 residual ranks are also
indistinguishable from zero. The retained checkpoint cannot report how many
stories use date-only timing. Nor does the aggregate evidence provide a
common-universe era estimate or a decomposition of firm, news, class and timing
composition. The era contrast therefore cannot distinguish market instability
from changing sample or measurement. LSEG has precise availability but a
different return outcome, so cross-source coefficients are not identical
estimands.
...
association with abnormal returns in FNSPID development. The development
baseline coefficient is $-0.00831$, and the separate recent-price-control
robustness estimate is $-0.00914$. The frozen 2020--2023 opening repeats the
baseline specification and gives $+0.00583$. The direct contrast therefore
supports instability in the measured baseline pipeline association; the
evidence does not identify whether market conditions, sample composition,
timing or classifier measurement caused the change. The tested daily
implementation does not cover realistic costs. LSEG estimates are imprecise,
and tuned LSEG risk and prompt rules fail their economic gates.
````
...
manuscript/artifacts/fig_timing_placebo.png	present	fabfa0ef8c1457f2d232297bed86c0efc44f171502db3238e97a9d0cfba40113	84326
...
manuscript/ucl_logo.jpg	absent	-	0
manuscript/ucl_logo.svg	present	be547f3dbc541c189d102696ac0ba90a206204a3ba6202f67189d74acfecbc3e	1706
````
