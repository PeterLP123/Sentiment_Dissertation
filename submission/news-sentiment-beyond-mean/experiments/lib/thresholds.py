...
def gate_daily_portfolio(
    ...
    *,
    ...
    asset_return_col: str = "ret_open_h1",
) -> pd.DataFrame:
    ...
    for session, day in use.groupby("session_date", sort=True):
        ...
        names = set(previous_drifted) | set(current)
        turnover = 0.5 * sum(
            abs(current.get(s, 0.0) - previous_drifted.get(s, 0.0)) for s in names
        )
        ...
        try:
            previous_drifted = drifted_weight_map(
                symbols,
                weights,
                day[asset_return_col].to_numpy(dtype=float),
            )
    ...
    if rows and previous_drifted:
        liquidation_turnover = 0.5 * sum(
            abs(weight) for weight in previous_drifted.values()
        )
